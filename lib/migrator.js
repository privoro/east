...
Migrator.prototype.migrationPrerequisite = function(callback) {
	var self = this;
	if (self.adapter.config && self.adapter.config.migrationTable === '_seeds') {
		self.adapter.resetExecuted(callback);
	}
	callback();
};
